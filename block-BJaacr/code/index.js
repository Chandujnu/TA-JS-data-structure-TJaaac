...
// 4. Log the name of all the people in house Starks (index 0) one by one
  ...
  for(let peopleOfHouse of got.houses[0].people) {
    console.log(peopleOfHouse.name);
  }

// 5. Log the name of all the people in house Lannisters (index 1) one by one

for(let peopleOfLannister of got.houses[1].people){
  console.log(peopleOfLannister.name);
}
// 6. Log the description of all the people in house Lannisters (index 1) one by one

for(let desOfPeople of got.houses[1].people) {
  console.log(desOfPeople.description);
}
...
for(let nameAndDesOfPeopleOfLannisters of got.houses[1].people) {
  console.log(`I am ${nameAndDesOfPeopleOfLannisters.name} and my bio is ${nameAndDesOfPeopleOfLannisters.description}`);
}
...
for(let nameAndDesOfPeopleOfStarks of got.houses[0].people) {
  console.log(`I am ${nameAndDesOfPeopleOfStarks.name} and my bio is ${nameAndDesOfPeopleOfStarks.description}`);
}

// 9. Log the name and description of the people in house Starks whose name is `Robb Stark`

for(let peopleOfStarks of got.houses[0].people) {
 if(peopleOfStarks.name === "Robb Stark") {
   console.log(peopleOfStarks.name, peopleOfStarks.description);
 }
}

// 10. Log the name and description of the people in house Lannisters whose name is `Tywin Lannister`

for(let personOfLannister of got.houses[1].people) {
  if(personOfLannister.name == "Tywin Lannister") {
    console.log(personOfLannister.name, personOfLannister.description);
  }
}
...
let obj = {};
for(let nameAndSizeOfPeople of got.houses) {
  obj[nameAndSizeOfPeople.name] = nameAndSizeOfPeople.people.length;
}
console.log(obj);

// 15. Log the name of all the people of all the houses in got one by one
for(let house of got.houses) {
  for(let nameOfAllPeople of house.people){
    console.log(nameOfAllPeople.name);
  }
}
...
console.log(allDescription.lenght > 30);
